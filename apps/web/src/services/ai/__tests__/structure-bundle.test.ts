...
describe('structure-bundle', () => {
  ...
  it('encodes a zip with manifest, raw view, sanitized view, and assets only', async () => {
    ...
    expect(entryNames.some((name) => name.endsWith('.tsx') || name.endsWith('.vue') || name.endsWith('.html'))).toBe(false)

    const manifest = JSON.parse(new TextDecoder().decode(archive['manifest.json'])) as AIStructureBundleManifest
    const rawView = JSON.parse(new TextDecoder().decode(archive['views/raw.json'])) as AIStructureBundleViewFile
    const sanitizedView = JSON.parse(new TextDecoder().decode(archive['views/sanitized.json'])) as AIStructureBundleViewFile
