...
export function getEditablePathState(
  node: PathNode,
  frame: PathFrame,
): EditablePathState | null {
...
export function mapAnchorsToScene(
  ...
  return anchors.map((anchor) => ({
    ...
    handleIn: anchor.handleIn
      ? { x: anchor.handleIn.x * sx, y: anchor.handleIn.y * sy }
      : null,
    handleOut: anchor.handleOut
      ? { x: anchor.handleOut.x * sx, y: anchor.handleOut.y * sy }
      : null,
...
function applyMirroredPointType(
  ...
  const preferredDirection =
    normalizeVector(anchor.handleOut)
    ?? invertVector(normalizeVector(anchor.handleIn))
    ?? normalizeVector(defaults.handleOut)
    ?? invertVector(normalizeVector(defaults.handleIn))
    ?? { x: 1, y: 0 }

  const mirrorLength = average(
    [
