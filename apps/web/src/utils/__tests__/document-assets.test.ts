...
describe('document asset paths', () => {
  ...
  it('keeps remote and inline assets unchanged at runtime', () => {
    expect(
      resolveRuntimeAssetSource('https://example.com/hero.png', null),
    ).toEqual({
      sourcePath: null,
    ...
    expect(
      resolveRuntimeAssetSource('data:image/png;base64,abc', null),
    ).toEqual({
      sourcePath: null,
  ...
  it('marks relative assets unresolved when there is no document path', () => {
    expect(
      resolveRuntimeAssetSource('./assets/hero.png', null),
    ).toEqual({
      sourcePath: null,
      runtimeUrl: null,
      isLocal: true,
  ...
  it('bridges local assets through the app origin when running over http', () => {
    ...
    try {
      expect(
        toLocalAssetBridgeUrl('C:/Users/fangx/Desktop/project/assets/hero image.png'),
      ).toBe(
