...
function toRelativePath(baseDir: string, targetPath: string): string | null {
  ...
  const baseParts = stripPrefix(normalizedBase, basePrefix)
    .split('/')
    .filter(Boolean)
  const targetParts = stripPrefix(normalizedTarget, targetPrefix)
    .split('/')
    .filter(Boolean)
...
function createLocalRuntimeAssetSource(sourcePath: string): RuntimeAssetSource {
  return {
    sourcePath,
    runtimeUrl: canBridgeLocalImagePath(sourcePath)
      ? toRuntimeLocalAssetUrl(sourcePath)
      : null,
    isLocal: true,
    unresolved: false,
  }
...
function joinNormalizedPath(baseDir: string, relativePath: string): string {
  ...
  const baseSegments = stripPrefix(normalizedBase, prefix)
    .split('/')
    .filter(Boolean)
