...
export function prepareImportedDocument(
  ...
  const appliedLegacyPenCompatibility = shouldApplyLegacyPenCompatibility(
    raw,
    options,
  )

  return {
    doc: appliedLegacyPenCompatibility
      ? applyLegacyPenCompatibility(normalized)
      : normalized,
