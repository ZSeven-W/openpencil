...
describe('CodePanel export affordances', () => {
  ...
  it('shows repair attempt issues in the current result and history entry', async () => {
    listCodeGenerationHistoryMock.mockResolvedValue([
      {
        id: 'gen-repaired',
        fileId: 'file-1',
        ...
        documentRevision: 3,
        status: 'done',
        finalCode: 'export default function Repaired() { return null; }',
        degraded: false,
        assetsManifest: [],
        model: 'repair-model',
        provider: 'builtin',
          ...
          repairAttempts: [
            {
              ...
              issues: [
                {
                  code: 'missing_text',
                  severity: 'error',
                  message: 'Missing CTA copy',
                },
              ],
              report: {
                status: 'repaired',
                framework: 'react',
                checkedAt: '2026-05-09T11:00:01.000Z',
                issues: [],
                summary: {
                  fileCount: 1,
                  errorCount: 0,
              ...
              code: 'export default function Repaired() { return null; }',
            },
          ],
        },
        createdAt: '2026-05-09T11:00:00.000Z',
    ...
    await waitFor(() => {
      expect(screen.getByText(/function Repaired/i)).toBeTruthy();
    });

    expect(screen.getByText('Repair details')).toBeTruthy();
    expect(screen.getAllByText('missing_text').length).toBeGreaterThan(0);
    expect(screen.getAllByText('Missing CTA copy').length).toBeGreaterThan(0);

    fireEvent.click(screen.getByRole('button', { name: /^History/i }));

    expect(screen.getByText(/repair issues: missing_text/i)).toBeTruthy();
  });
