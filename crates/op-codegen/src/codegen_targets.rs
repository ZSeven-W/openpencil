...
use crate::{
    ...
    node_hidden, node_is_ellipse, node_is_text, node_origin, node_size, node_stroke_css, node_text,
    parse_color, root_nodes, Codegen, CssVariables,
};
...
#[path = "codegen_html.rs"]
mod codegen_html;
#[path = "codegen_motion_css.rs"]
mod codegen_motion_css;
use codegen_html::emit_node_html;
pub use codegen_html::Html;
...
pub(super) fn widget_markup(node: &PenNode) -> Option<String> {
    Some(match node {
...
pub(super) fn tabs_nav_markup(node: &PenNode) -> Option<String> {
...
pub(super) fn emitted_corner_radius(node: &PenNode) -> f64 {
...
pub(super) fn image_object_fit(node: &PenNode) -> Option<&'static str> {
    let PenNode::Image(image) = node else {
        return None;
    };
