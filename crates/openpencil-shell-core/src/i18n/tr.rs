//! Generated from `apps/web/src/i18n/locales/tr.ts` —
...
pub fn lookup(key: &str) -> Option<&'static str> {
    Some(match key {
        ...
        "git.conflict.banner.pollError" => {
            "Bir hata nedeniyle durum kontrolü duraklatıldı: {{message}}"
        }
        ...
        "git.picker.empty.body" => {
            "Git paneli çalışmak için en az bir .op dosyasına ihtiyaç duyar."
        }
        ...
        "git.wizard.clone.anonymousHint" => {
            "Anonim klonlamak için boş bırakın (yalnızca genel depolar)."
        }
        ...
        "git.wizard.clone.validationTokenUsername" => {
            "Belirteç sağlandığında kullanıcı adı gerekli"
        }
        ...
        "builtin.description" => {
            "API anahtarlarını doğrudan yapılandırın — CLI araçlarına gerek yok."
        }
        ...
        "landing.shortcutHint" => {
            "Yeni tasarım oluşturmak için {{key1}} + {{key2}} tuşlarına basın"
        }
