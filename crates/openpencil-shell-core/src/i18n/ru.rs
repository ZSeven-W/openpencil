...
pub fn lookup(key: &str) -> Option<&'static str> {
    Some(match key {
        ...
        "git.wizard.clone.validationTokenUsername" => {
            "Имя пользователя обязательно при указании токена"
        }
        ...
        "git.wizard.clone.error.clone-target-exists" => {
            "Папка назначения уже существует или не пуста."
        }
        "git.pull.label" => "Pull",
        ...
        "builtin.parallelAgents" => {
            "Параллельные суб-агенты: {{count}}x (нажмите для переключения)"
        }
