...
/// A bottom-tab-bar shell the model never filled gets the standard generic
/// 3-tab set instead of shipping as an empty grey strip (measured twice,
/// test0711-22). Desktop roots and already-filled navs are untouched.
#[test]
fn empty_bottom_nav_shell_is_filled_with_generic_tabs() {
    let doc: jian_ops_schema::PenDocument = serde_json::from_str(
    ...
    let root = &state.active_children()[0];
    let nav = find_by_id(root, "nav").expect("nav");
    let tabs = nav.children().expect("tabs");
    assert_eq!(tabs.len(), 3, "generic Home/Search/Profile set inserted");
    assert!(tabs
        .iter()
        .all(|tab| tab.children().is_some_and(|c| c.len() == 2)));
}
