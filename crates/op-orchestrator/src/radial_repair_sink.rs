//! Late, sink-based radial repair against RESOLVED layout rects.
//!
//! The pure authored-forest passes in the parent module run before
//! `InsertSubtree`; this is the fallback that re-centres rings once the tree
//! is live in an `EditorState` and real resolved geometry (platform text
//! growth included) is available. Lane ownership with `ring_repair` is
//! decided by [`super::parent_is_dedicated_ring_wrapper`].
...
use super::{
    children, estimated_subtree_size, estimated_text_size, has_nonzero_padding, has_numeric,
    is_arc_ellipse, near_square, numeric, radial_layer_order, radial_layers, valid_size,
    MAX_SAFE_ARC_TO_PARENT_RATIO, MIN_SAFE_ARC_DIAMETER_RATIO, MIN_SAFE_ARC_TO_PARENT_RATIO,
};
...
pub fn repair_radial_stacks(sink: &mut dyn crate::types::DocSink, root_id: &str) -> bool {
...
pub(super) fn radial_stack_repair(
    v: &Value,
    rects: &HashMap<String, Rect>,
    ...
    // Multi-centre stacks: centre-content children keep their authored
    // positions (see `authored_radial_patch`); only the arcs are centred.
    let keep_authored_centres = radial_layers(v).is_some_and(|layers| layers.centres.len() > 1);
    ...
    cmds.extend(
        super::radial_repair_partial_pair::canonical_reorder_commands(id, kids, &layer_order),
    );
    if fix_parent_width {
    ...
    for child in kids {
        ...
        let position = !keep_authored_centres || is_arc_ellipse(child);
        if position {
