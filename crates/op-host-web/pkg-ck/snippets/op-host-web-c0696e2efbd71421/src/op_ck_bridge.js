...
export async function opCkInit(canvasId, roboto, cjk) {
  ...
  return {
    ...
    drawText(t, x, y, sz, weight, italic, r, g, b, a) {
      ...
      canvas.drawText(t, x, y, p, f);
      p.delete(); f.delete();
    },
    measureText(t, sz) { const f = fontFor(t, sz); const ids = f.getGlyphIDs(t); const w = f.getGlyphWidths(ids).reduce((s, v) => s + v, 0); f.delete(); return w; },
