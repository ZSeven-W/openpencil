...
- **Web bundle ceiling: 6 MiB gzip + 0 env.\* imports.** Enforced by `tools/check-wasm-bundle.sh`. The CanvasKit bundle carries the full app logic (codegen AI pipeline, Figma parser, AI/live-sync, collaboration) plus the still-embedded AI skill corpus (~1.1 MiB of markdown via `include_dir!`), so the ceiling sits well above the retired skia raster path's 1 MiB (**4.99 MiB today**, measured 2026-09-26 after the schema serde de-duplication cut ~0.7 MiB; the ceiling is a runaway-regression tripwire, not a budget). Re-baselined from 8 MiB once the ~2.4 MiB of preview JPEGs moved out — see below. The 0 env.\* guard still holds — CanvasKit needs no libc shim.
- **Product assets are embedded on native and fetched on wasm.** `op-editor-core/src/web_assets.rs` is the platform-free half: a process-global `route -> bytes` registry with single-flight, a widget-records / host-drains request queue, and `Absent/Pending/Ready/Failed` states. The widget layer calls `web_assets::request(route)` from paint and draws a placeholder; `op-host-web/src/web_asset_fetch.rs` drains it once per frame over XHR and installs the bytes. Native keeps its `include_bytes!` unchanged behind `#[cfg(not(target_arch = "wasm32"))]`, so desktop behaviour is byte-identical. Assets are served from `/pkg/assets/…` (**not** `/assets/`, which belongs to the hub frontend) and staged into the bundle by `tools/stage-web-assets.sh`, which `check-wasm-bundle.sh` runs as step 4 and both the CI workflow and `Dockerfile.web-rust` inherit. Moved: the Prompt Center (~2.1 MiB) and 96 Scene Template preview JPEGs (~4.4 MiB), the 96 scene-template `.op` documents (~7.8 MiB), and `iconify-catalog-core.json` (~452 KiB) — ~14.8 MiB of staged assets in total. Still embedded everywhere: the `op-ai-skills` corpus (~1.1 MiB, `include_dir!`).
- **The icon-catalog split is opt-in, the rest is `cfg(target_arch = "wasm32")`.** `op-editor-ui`'s `runtime-icon-catalog` feature is enabled only by `op-host-web`, which has a daemon to fetch from and a frame pump to install on. `op-web-sdk` renders iconFont nodes but has no daemon, so it keeps the embed — for that viewer "not fetched yet" would mean "never". Bundle ceilings are per-bundle and measured: op-host-web 6 MiB (4.99 MiB measured 2026-09-26), op-web-sdk 6 MiB (5.20 MiB measured).
...
- **Lean profile** (`mcp_serve/tool_catalog.rs` + `lean_profile.rs` + `lean_tools.rs`): an opt-in six-tool catalog — `get_editor_state` → `get_guidelines` → `batch_design` → `snapshot_layout` / `get_screenshot` → `finalize_design` — selected by `--mcp-profile lean` (or `OPENPENCIL_MCP_PROFILE=lean`) on `--mcp` / `--mcp-http`, or the `/mcp/lean` HTTP path (live desktop server and `--mcp-http`). Schemas are derived from `TOOL_SCHEMAS` with workflow-step descriptions; lean `get_editor_state` composes doc info / pages / variables / theme and forwards `nodeIds`/`depth` to `read_nodes`, lean `get_screenshot` forwards `format`/`scale` to `export_nodes`. Anything else is refused with `tool-not-in-profile` before it runs. The settings MCP tab's "Lean tool profile" switch makes the integration writer install `/mcp/lean`. The full catalog stays the default.
