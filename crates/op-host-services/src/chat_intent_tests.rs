...
fn classification_tag_parsing_matches_ts() {
    ...
    // Unknown / empty → chat: a classifier failure must not mutate the canvas.
...
#[test]
fn llm_classifier_keeps_a_complete_tag_received_before_timeout() {
    struct PartialThenSlow;
    impl ChatProvider for PartialThenSlow {
        fn provider_label(&self) -> &str {
            "partial-then-slow"
        }
