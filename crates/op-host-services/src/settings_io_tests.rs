...
fn atlas_image_generation_profile_round_trips_through_payload() {
    ...
    let payload = op_editor_host_core::settings_payload::image_gen_profile_to_payload(&profile);
    assert_eq!(payload.provider, "atlas");
    let restored = op_editor_host_core::settings_payload::image_gen_profile_from_payload(payload)
