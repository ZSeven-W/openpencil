...
fn tools_list_response(id_raw: &str) -> String {
    ...
    // client to render a tool picker + validate calls. Debug-tool
    // schemas are appended only when the isolation flag is set, so
    // they stay invisible to a production client.
    let mut schemas: Vec<&str> = TOOL_SCHEMAS.to_vec();
    if debug_tools_enabled() {
        schemas.extend_from_slice(DEBUG_TOOL_SCHEMAS);
    }
    format!(
