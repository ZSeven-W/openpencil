...
impl DesktopApp {
    ...
    pub(crate) fn reconcile_mcp_server_from_settings(&mut self) -> bool {
        ...
        match op_host_services::mcp_live::McpLiveServer::start_with_wake(port, self.mcp_wake_callback()) {
            Ok(server) => {
            ...
            Err(err) => {
                ...
                if port != 0 {
                    match op_host_services::mcp_live::McpLiveServer::start_with_wake(0, self.mcp_wake_callback()) {
                        Ok(server) => {
                            let bound_port = server.port();
                            eprintln!(
