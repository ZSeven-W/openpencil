...
impl DesktopCollabRuntime {
    pub(crate) fn poll(&mut self, host: &mut WidgetHostNative) -> bool {
        self.poll_with_hooks(host, || {}, DesktopCollabRuntime::launch_pending_network)
    }
    ...
    fn poll_with_hooks(
        ...
        launch_pending_network: impl FnOnce(&mut DesktopCollabRuntime) -> bool,
    ) -> bool {
    ...
    #[cfg(test)]
    pub(in crate::collab_runtime) fn poll_with_after_initial_data_drain(
        &mut self,
        host: &mut WidgetHostNative,
        after_initial_data_drain: impl FnOnce(),
    ) -> bool {
        self.poll_with_hooks(
            host,
            after_initial_data_drain,
            DesktopCollabRuntime::launch_pending_network,
        )
    }

    #[cfg(test)]
    pub(in crate::collab_runtime) fn poll_with_launch_probe(
        &mut self,
        host: &mut WidgetHostNative,
        launch_probe: impl FnOnce(&mut DesktopCollabRuntime) -> bool,
    ) -> bool {
