...
function generateNodeHTML(node: PenNode, depth: number, rules: CSSRule[]): string {
  ...
  switch (node.type) {
    ...
    case 'group': {
      ...
      const childrenHTML = children.map((c) => generateNodeHTML(c, depth + 1, rules)).join('\n');
    ...
    case 'image': {
      ...
      if (typeof node.height === 'number') css.height = `${node.height}px`;
      const fit =
        node.objectFit === 'fit' ? 'contain' : node.objectFit === 'crop' ? 'cover' : 'fill';
      css['object-fit'] = fit;
      Object.assign(css, cornerRadiusToCSS(node.cornerRadius));
      Object.assign(css, effectsToCSS(node.effects));
...
export function generateHTMLCode(nodes: PenNode[]): { html: string; css: string } {
  ...
  const childrenHTML = nodes.map((n) => generateNodeHTML(n, 1, rules)).join('\n');
...
export function generateHTMLFromDocument(
  ...
  const varsCSS =
    doc.variables && Object.keys(doc.variables).length > 0 ? generateCSSVariables(doc) : '';
  return {
    html: result.html,
