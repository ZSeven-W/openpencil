...
function strokeToSwiftUI(
  ...
  if (cr > 0) {
    modifiers.push(
      `.overlay(RoundedRectangle(cornerRadius: ${cr}).stroke(${strokeColor}, lineWidth: ${thicknessStr}))`,
    );
  } else {
...
function effectsToSwiftUI(effects: PenEffect[] | undefined): string[] {
  ...
  for (const effect of effects) {
    if (effect.type === 'shadow') {
      ...
      modifiers.push(
        `.shadow(color: ${hexToSwiftUIColor(s.color)}, radius: ${s.blur}, x: ${s.offsetX}, y: ${s.offsetY})`,
      );
...
function alignToSwiftUI(alignItems: string | undefined, layout: string | undefined): string | null {
...
function renderWithModifiers(pad: string, element: string, modifiers: string[]): string {
...
function generateContainerSwiftUI(node: PenNode & ContainerProps, depth: number): string {
  const pad = indent(depth);
...
export function generateSwiftUICode(nodes: PenNode[], viewName = 'GeneratedView'): string {
  if (nodes.length === 0) {
  ...
  const childLines = nodes.map((n) => generateNodeSwiftUI(n, 3)).join('\n');
