...
function strokeToStyle(stroke: PenStroke | undefined): Record<string, string> {
  ...
  if (typeof stroke.thickness === 'string' && isVariableRef(stroke.thickness)) {
    ...
    const thickness = typeof stroke.thickness === 'number' ? stroke.thickness : stroke.thickness[0];
...
function generateNodeRN(node: PenNode, depth: number): string {
  ...
  switch (node.type) {
    ...
    case 'group': {
      ...
      const childrenJSX = childNodes.map((c) => generateNodeRN(c, depth + 1)).join('\n');
      return `${comment}${pad}<View style=${formatStyle(styles)}>\n${childrenJSX}\n${pad}</View>`;
    }
    ...
    case 'line': {
      ...
      if (lineNode.stroke) {
        const thickness =
          typeof lineNode.stroke.thickness === 'number'
    ...
    case 'path': {
      ...
      const fillColor =
        pathNode.fill?.[0]?.type === 'solid'
          ? varOrLiteral(pathNode.fill[0].color)
          : 'currentColor';
      const posStyles = commonStyles(node);
      const posStr = Object.keys(posStyles).length > 0 ? ` style=${formatStyle(posStyles)}` : '';
    ...
    case 'polygon': {
      ...
      const fillColor =
        polyNode.fill?.[0]?.type === 'solid' ? varOrLiteral(polyNode.fill[0].color) : 'none';
    ...
    case 'icon_font': {
      ...
      const color =
        node.fill?.[0]?.type === 'solid' ? varOrLiteral(node.fill[0].color) : 'currentColor';
...
export function generateReactNativeCode(
  ...
  if (needsSvg) {
    ...
    lines.push(
      `import Svg, { ${svgImports.filter((s) => s !== 'default as Svg').join(', ')} } from 'react-native-svg'`,
    );
  }
  ...
  const childrenJSX = nodes.map((n) => generateNodeRN(n, 2)).join('\n');
