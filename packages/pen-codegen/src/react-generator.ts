...
function generateNodeJSX(node: PenNode, depth: number): string {
  ...
  switch (node.type) {
    ...
    case 'group': {
      ...
      const childrenJSX = childNodes.map((c) => generateNodeJSX(c, depth + 1)).join('\n');
...
export function generateReactCode(nodes: PenNode[], componentName = 'GeneratedDesign'): string {
  if (nodes.length === 0) {
  ...
  const childrenJSX = nodes.map((n) => generateNodeJSX(n, 2)).join('\n');
