...
function cornerRadiusToFlutter(
  ...
  const thickness =
    typeof stroke.thickness === 'number'
      ...
      : typeof stroke.thickness === 'string'
        ...
        : stroke.thickness[0];
  const thicknessStr =
    typeof thickness === 'string' && isVariableRef(thickness)
...
function effectsToBoxShadows(effects: PenEffect[] | undefined): string[] {
  ...
  for (const effect of effects) {
    if (effect.type === 'shadow') {
      ...
      shadows.push(
        `BoxShadow(color: ${hexToFlutterColor(s.color)}, blurRadius: ${s.blur}, offset: Offset(${s.offsetX}, ${s.offsetY}))`,
      );
    }
  }
  return shadows;
...
function paddingToFlutter(
  ...
  if (typeof padding === 'string' && isVariableRef(padding))
    return `EdgeInsets.all(/* ${varOrLiteral(padding)} */ 0)`;
  if (typeof padding === 'number') return padding > 0 ? `EdgeInsets.all(${padding})` : null;
  if (Array.isArray(padding)) {
    if (padding.length === 2)
      return `EdgeInsets.symmetric(vertical: ${padding[0]}, horizontal: ${padding[1]})`;
    if (padding.length === 4) {
...
function generateNodeFlutter(node: PenNode, depth: number): string {
  switch (node.type) {
    ...
    case 'rectangle':
    case 'group':
      return generateContainerFlutter(node, depth);
    case 'ellipse':
      return generateEllipseFlutter(node as EllipseNode, depth);
    case 'text':
      return generateTextFlutter(node as TextNode, depth);
    case 'line':
      return generateLineFlutter(node as LineNode, depth);
    case 'path':
      return generatePathFlutter(node as PathNode, depth);
    case 'polygon':
      return generatePolygonFlutter(node as PolygonNode, depth);
    case 'image':
      return generateImageFlutter(node as ImageNode, depth);
    case 'icon_font': {
    ...
    case 'ref':
      return `${indent(depth)}// Ref: ${(node as any).ref}`;
    default:
...
function generateContainerFlutter(node: PenNode & ContainerProps, depth: number): string {
  ...
  const decoration = buildBoxDecoration(
    node.fill,
    node.cornerRadius,
    node.stroke,
    node.effects,
...
function generateTextFlutter(node: TextNode, depth: number): string {
  ...
  if (decorations.length === 1) styleParts.push(`decoration: ${decorations[0]}`);
    ...
    styleParts.push(`decoration: TextDecoration.combine([${decorations.join(', ')}])`);
...
export function generateFlutterCode(nodes: PenNode[], widgetName = 'GeneratedDesign'): string {
  if (nodes.length === 0) {
...
export function generateFlutterFromDocument(
  ...
  const children =
    activePageId !== undefined ? getActivePageChildren(doc, activePageId) : doc.children;
