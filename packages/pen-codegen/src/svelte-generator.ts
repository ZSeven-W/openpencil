...
function effectsToCSS(effects: PenEffect[] | undefined): Record<string, string> {
  ...
  for (const effect of effects) {
    if (effect.type === 'shadow') {
      ...
      shadows.push(
        `${s.inner ? 'inset ' : ''}${s.offsetX}px ${s.offsetY}px ${s.blur}px ${s.spread}px ${s.color}`,
      );
    }
  }
...
function layoutToCSS(node: ContainerProps): Record<string, string> {
  ...
  if (node.gap !== undefined) {
    css.gap =
      typeof node.gap === 'string' && isVariableRef(node.gap)
  ...
  switch (node.type) {
    ...
    case 'group': {
      ...
      Object.assign(
        css,
        fillToCSS(node.fill),
        strokeToCSS(node.stroke),
        cornerRadiusToCSS(node.cornerRadius),
      ...
      const childrenMarkup = children
        .map((c) => generateNodeMarkup(c, depth + 1, rules))
        .join('\n');
    ...
    case 'text': {
      ...
      if (node.fill) {
        const f = node.fill[0];
        if (f?.type === 'solid') css.color = varOrLiteral(f.color);
      }
    ...
    case 'line': {
      ...
      if (node.stroke) {
        ...
        if (node.stroke.fill?.[0]?.type === 'solid')
          css['border-top-color'] = varOrLiteral(node.stroke.fill[0].color);
      }
