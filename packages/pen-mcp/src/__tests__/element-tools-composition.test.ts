...
describe('element-tools composition — N-tool real screens', () => {
  it(
    'mobile settings: top nav + 2 sections × 3 list rows + bottom nav (9 calls)',
    async () => {
  ...
  it(
    'dashboard home: nav + stat grid + section + 3 metric comparisons + chart (7 calls)',
    async () => {
  ...
  it(
    'login form: heading + body + 2 fields + button + link (6 calls)',
    async () => {
  ...
  it(
    'profile + UGC: nav + avatar + heading + badge + 2 FAQ items + action menu (7 calls)',
    async () => {
  ...
  it(
    'listing: search + card row + divider + empty chart + date picker + chip input + pagination (7 calls)',
    async () => {
  ...
  it(
    'parent_id threading: nested insert actually lands under named parent',
    async () => {
      const fp = await fresh('settings.op');
  ...
  it(
    'support chat: exercises every tool added after the 62-mark (#63-#67 + v1)',
    async () => {
