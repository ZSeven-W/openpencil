/**
 * Pure batch_design DSL executor. Takes a `PenDocument` + DSL string,
 * mutates the doc in place via pen-core's tree ops, returns results +
 * per-line errors. No file I/O, no network access unless the caller
 * supplies `imageSearchFetcher` for the `G` op. Browser-safe.
 *
 * This file used to live inside `batch-design.ts` alongside
 * `handleBatchDesign` (the server-side wrapper that opens + saves a
 * .op file). It was extracted so apps/web can run the DSL directly
 * in the browser without pulling node:fs via the document-manager
 * import chain. pen-mcp barrel still re-exports `runBatchDesignDsl`
 * (see `src/index.ts`) — the public API is unchanged.
 */
...
/**
 * Optional hook for the `G` (generate/search image) op. When absent,
 * the op still emits an image node but leaves `src` empty — the
 * caller can enrich via a separate pass. When present, the fetcher
 * is called with the prompt; if it returns a URL, `src` is set.
 *
 * Server-side callers (pen-mcp handleBatchDesign) pass a fetcher
 * that resolves `getSyncUrl()` + POSTs to /api/ai/image-search.
 * Browser-side callers can pass a fetcher that uses a relative URL,
 * or omit the fetcher entirely.
 */
export type ImageSearchFetcher = (prompt: string) => Promise<string | null>;

export interface RunBatchDesignDslOptions {
  pageId?: string;
  imageSearchFetcher?: ImageSearchFetcher;
}

export async function runBatchDesignDsl(
  doc: PenDocument,
  operations: string,
  opts: RunBatchDesignDslOptions = {},
): Promise<{
  results: OpResult[];
  ...
  for (const line of lines) {
    try {
      await executeLine(line, doc, bindings, results, opts.pageId, opts.imageSearchFetcher);
    } catch (err) {
 ...
 */
export function splitOperations(raw: string): string[] {
  const result: string[] = [];
  let buf = '';
  let depth = 0;
  let inString = false;
  ...
  if (effectiveAssign) {
    ...
    switch (op) {
      ...
      case 'G': {
        ...
        // Ask the injected fetcher for a URL. When no fetcher is
        // supplied (e.g. browser caller that skips image search for
        // now), the image node is emitted with empty src and the
        // caller's own pipeline enriches later.
        if (gMode === 'search' && imageSearchFetcher) {
          try {
            const src = await imageSearchFetcher(gPrompt);
            if (src) imageNode.src = src;
          } catch {
            /* keep empty src */
          }
        }
        // "generate" mode is not supported at this layer — needs
        // provider API keys. Callers that want generate mode must
        // handle it in a separate pipeline.
...
export function countNodes(nodes: PenNode[]): number {
  let count = 0;
  for (const node of nodes) {
    count++;
    if ('children' in node && node.children) {
